 ...
 * supporting multiple controller soon. Additionally, it's important to note
 * that bit bang is a best try implementation of i2c. As in it will do it's best
 * to reach the requested frequency but will typically be slower then requested
 * with brief testing it seems that the error is about 2us per period for the
 * lpc4078 , so you can try to account for this error with the frequency you set
 * for example, achieving the correct period for 100kHz would result in setting
 * slightly more then a 125kHz frequency. Additionally, the fastest frequency
 * this bit bang can handle is around 200kHz.
 */
...
public:
   ...
   * @param p_bus the bus information which will be used which configuring the
   * bit bang interface
   */

  i2c_bit_bang(i2c_pins p_pins, steady_clock& p_steady_clock, bus_info p_bus);
...
private:
  /**
   * @brief this function will send the start condition and it will always pull
  ...
  /**
   * @brief this function will go through the steps of writing the address of
   * the peripheral the controller wishes to speak to while also ensuring the
   ...
   *
   * @throws hal::io_error when the address written to the bus was not
   * acknowledged
   */
  ...
  /**
   * @brief this function is responsible for reading a single bit at a time off
   * of the bus while also managing the clock line. It will release (pull sda
   ...
   * which operates on ticks rather then a time duration where hal::delay
   * provides delays up to 17us, this function will provide delays up to
   * about 2.7us making it optimal for high accuracy delay
   *
   * @param ticks the amount of ticks this function will delay for, this should
   * be based on the frequency of the clock passed into the bit_bang class
   */
  ...
  /**
   * @brief the amount of ticks that the uptime function takes to run, we store
   * this so we save computation time in our high speed delay function
   */
